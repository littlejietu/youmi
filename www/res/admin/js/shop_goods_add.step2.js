$(function(){
    ...
    // 计算利润价
    $('input[name="comm_precent"],input[name="cost_price"]').change(function(){
        commpriceCalculator();
    });
    ...
    $('a[nctype="mb_add_img"]').click(function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('/admin/goods_tpl_album/pic_list?item=mobile');
    });
    // 关闭 图片选择
...
// 计算利润
function commpriceCalculator() {
    var _price = parseFloat($('input[name="price"]').val());
    var _costprice = parseFloat($('input[name="cost_price"]').val());
    var _commprecent = parseFloat($('input[name="comm_precent"]').val());
    if((!isNaN(_price) && _price != 0) && (!isNaN(_costprice) && _costprice != 0)){
        var _commprice = number_format( (_price-_costprice)*_commprecent/100, 2);
        if(_commprice<0)
            _commprice = 0;
        $('input[name="comm_price"]').val(_commprice);
    }
    else
        $('input[name="comm_price"]').val(0);
}
