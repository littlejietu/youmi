...
class Home extends BaseSellerController {
			...
			'left' =>array(
				...
				1 => array(
					...
					'list' => array(
						...
						array('args'=>'onekey,bind,setting',							'text'=>'微信一键绑定'),
				...
				5 => array(
					...
					'list' => array(
						...
						array('args'=>'cashier,report,trade',							'text'=>'收银员查询'),
						array('args'=>'customer_oil,report,trade',							'text'=>'客单量查询'),
						array('args'=>'rfm,report,trade',							'text'=>'消费频次查询'),
	...
	private final function parseMenu($menu = array()){
		$admin_info = $this->getSellerInfo();
		if ($admin_info['is_super'] == 1) return $menu;
