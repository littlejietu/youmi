 ...
 * WEIZAN is NOT a free software, it under the license terms, visited http://chat.idear7.cn/ for more details.
 */
...
function file_upload($file, $type = 'image', $name = '') {
	...
	if (empty($name) || $name == 'auto') {
		...
		mkdirs(dirname($upload_dir . '/' . $name));
	...
	if (!file_move($file['tmp_name'], $upload_dir . '/' . $result['path'])) {
		return error(-1, '保存上传文件失败');
	}
	$result['success'] = true;
	return $result;
