...
class WeixinPayApp{
    ...
    public function payRequest($arrFundOrder, $wxConfig) {
		...
		if(!empty($result['return_code']) && $result['return_code']=='SUCCESS' && $result['result_code']=='SUCCESS')
		{
			$appParameters = $this->GetAppParameters($result, $wxConfig['KEY']);
...
//    }
	 ...
	 */
	public function GetAppParameters($UnifiedOrderResult, $KEY)
	{
		...
		$app_api->SetSign($app_api->MakeSign($KEY));
