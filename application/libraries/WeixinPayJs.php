...
class WeixinPayJs{
    ...
    public function payRequest($arrFundOrder, $wxConfig, $openId=null) {
    	...
    	if(empty($openId)){
			...
			{
				$jsApiParameters = $this->GetJsApiParameters($result, $wxConfig['KEY']);
    ...
    public function parseNotice($xml, $wxConfig){
    	if(empty($wxConfig))
    		return '未设置';
    	$arrPayNoticeResult = array();
    	try {
			$result = WxPayResults::Init($xml, $wxConfig['KEY']);
...
//    }
	 ...
	 * @return 用户的openid
	 */
	public function GetOpenid($fund_order_id=0, $wxConfig)
	{
		//通过code获得openid
		if (!isset($_GET['code'])){
			//触发微信返回code码
			$redirectUrl =  urlencode(BASE_SITE_URL.'/api/jump/getWxJSAPI?payMethod=12&fundId='.$fund_order_id);
			$url = $this->__CreateOauthUrlForCode($redirectUrl, $wxConfig);
//echo $url;die;
			...
			//获取code码，以获取openid
		    $code = $_GET['code'];
			$openid = $this->getOpenidFromMp($code, $wxConfig);
			return $openid;
		}
	}
	 ...
	 * @return openid
	 */
	public function GetOpenidFromMp($code, $wxConfig)
	{
		$url = $this->__CreateOauthUrlForOpenid($code, $wxConfig);
		//初始化curl
		$ch = curl_init();
		//设置超时
	 ...
	 * @return 返回构造好的url
	 */
	private function __CreateOauthUrlForCode($redirectUrl, $wxConfig)
	{
	 ...
	 * @return 请求的url
	 */
	private function __CreateOauthUrlForOpenid($code, $wxConfig)
	{
		$urlObj["appid"] = $wxConfig['APPID'];
